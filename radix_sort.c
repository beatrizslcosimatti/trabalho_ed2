...
void counting_sort_radix(int* arr, int tam, int algarismo, int* B, Metricas* m) {
    int C[10] = {0};
...
void Radix_sort(int* arr, int tam, Metricas* m) {
    if (tam <= 1) return;
    ...
    m->memoria_extra_bytes += (tam * sizeof(int)); 
    ...
    for(int algarismo = 1; max / algarismo > 0; algarismo *= 10) {
        counting_sort_radix(arr, tam, algarismo, B, m);
    }
